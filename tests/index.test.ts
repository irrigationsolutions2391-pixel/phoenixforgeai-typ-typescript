...
import { APIPromise } from 'phoenixforgeai-typ/core/api-promise';

import util from 'node:util';
import PhoenixforgeaiTyp from 'phoenixforgeai-typ';
import { APIUserAbortError } from 'phoenixforgeai-typ';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['PHOENIXFORGEAI_TYP_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: PhoenixforgeaiTyp) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['PHOENIXFORGEAI_TYP_LOG'] = 'debug';
      const client = new PhoenixforgeaiTyp({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['PHOENIXFORGEAI_TYP_LOG'] = 'not a log level';
      const client = new PhoenixforgeaiTyp({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'PHOENIXFORGEAI_TYP_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['PHOENIXFORGEAI_TYP_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new PhoenixforgeaiTyp({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['PHOENIXFORGEAI_TYP_BASE_URL'] = 'https://example.com/from_env';
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['PHOENIXFORGEAI_TYP_BASE_URL'] = ''; // empty
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['PHOENIXFORGEAI_TYP_BASE_URL'] = '  '; // blank
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['PHOENIXFORGEAI_TYP_BASE_URL'] = 'http://localhost:5000/env';
      const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new PhoenixforgeaiTyp({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new PhoenixforgeaiTyp({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new PhoenixforgeaiTyp({ apiKey: 'My API Key', fetch: testFetch });
